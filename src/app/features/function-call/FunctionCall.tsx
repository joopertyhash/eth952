...
const FunctionForm = ({ fn }) => {
  ...
  if (!fn) {
    return (
      <Container label="Функции вызова">
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.error(error);
      addLogItem(`Ошибка: ${error.message}`);
    }
  };
    ...
    <Container label="Функции вызова">
      <Content>
        ...
        {fn.stateMutability === "payable" && (
          <>
            <div>Эфир для отправки:</div>
            <Input
              type="number"
              placeholder="в единицах эфиров, а не вей"
              value={ethToSend}
        ...
        <div style={{ display: "flex" }}>
          ...
          />
          <GasLimitCheckbox
            label="пользовательский лимит газа"
        ...
        {showGasLimit && (
          <>
            ...
            <Input
              type="number"
              placeholder="оставьте пустым, чтобы использовать значение по умолчанию"
              value={gasLimit}
